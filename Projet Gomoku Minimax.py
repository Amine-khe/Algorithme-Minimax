...
def minimax_alpha_beta(etat, profondeur, alpha, beta, maxi):
    ...
    if maxi: #ici c'est pour le joueur maximinsant
        ...
        for action in actions(etat):
            ...
            score_actuel, _=minimax_alpha_beta(nouvel_etat,profondeur-1,alpha,beta,False)
            # Ici on met un appel récursif pour évaluer le score de cet état
...
def demander_coup_utilisateur(etat):
    while True:
        try:
            ...
            i,j=map(int,coup.split())  # On convertit l'entrée en deux entiers (ligne et colonne).
            coup=(i,j)
...
def jouer():
    ...
    while not Terminal_Test(etat):
        if etat["joueur"]==joueur_humain:
            ...
            print("En attente de la réponse de l'IA")
            if etat["tour"]==2 and joueur_humain==1 and not coup_defaut:
                action=(7,3)
                coup_defaut=True
            else:
                profondeur=2  # On initialise la profondeur de l'algorithme
